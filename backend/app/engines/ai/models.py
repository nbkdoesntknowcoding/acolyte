...
# ---------------------------------------------------------------------------
# 13. StudentRecommendation — Persisted recommendations from S6
#     Tenant-scoped. Multiple rows per student (active recommendations).
# ---------------------------------------------------------------------------

class StudentRecommendation(TenantModel):
    """Persisted recommendation from the S6 Recommendation Engine.

    Each recommendation has a lifecycle: active → dismissed/completed/expired.
    Dismissed/completed feedback is captured for future improvement.
    """
...
# ---------------------------------------------------------------------------
# 14. StudentStudyPlan — Weekly study plans from S6
#     Tenant-scoped. One active plan per student at a time.
# ---------------------------------------------------------------------------

class StudentStudyPlan(TenantModel):
    """Weekly study plan generated by the S6 Recommendation Engine."""
    ...
    focus_subjects = Column(JSONB, nullable=True)
    weekly_goal = Column(String(500), nullable=True)
    status = Column(String(20), nullable=False, server_default="active")
